...
public class GravelBeachImporter extends PolygonReadyVectorLoader {
	
	public GravelBeachImporter(double realworldSquareSize, double realworldCenterLong, double realworldCenterLat, int pixelMapSize, Path resourcesPath) throws MinecraftGenerationException {
		super(realworldSquareSize, realworldCenterLong, realworldCenterLat, pixelMapSize, resourcesPath);
		
		String fileName;
		String directory = "gravel-zones";
		...
		for(int x=0; x<valueMapSize; x++) {
			for(int y=0; y<valueMapSize; y++) {
				...
				if (valueMap[y*valueMapSize + x] != null
						&& map.getSurfaceBlock(x, y).canBeReplaced("GravelBeachImporter", x, y, valueMapSize) ) {
					map.setSurfaceBlock(x, y, SimpleBlocks.PUREGRAVEL.get());
				}
			}
		}
	}
